...
class PdfScansService {
  ...
  async createPdfScan(fileData, authorUsername) {
  ...
  async getAllPdfScans() {
    return await PdfScansRepo.findAll();
  }
  ...
  async runPdfScan(pdfScanId) {
    ...
    try {
      ...
      initialResult = await PdfScansRepo.createScanResult({
        pdfScanId,
        authorUsername: pdfScan.authorUsername,
